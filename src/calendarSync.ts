 ...
 */
function getDateFromSheet(): { startDate: Date; endDate: Date } {
 ...
 */
function getCalendarEvents(calendarId: string, startDate: Date, endDate: Date): Array<{
  title: string;
  startTime: Date;
  endTime: Date;
  location: string;
 ...
 * @param {Date} endDate - 終了日
 */
function exportCalendarToSheet(targetSheetName: string, calendarId: string, startDate: Date, endDate: Date): {
  sheetName: string;
  eventCount: number;
} {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
